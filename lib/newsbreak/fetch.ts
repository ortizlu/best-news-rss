...
const USER_AGENT =
  "Mozilla/5.0 (compatible; best-news-rss/1.0; +https://github.com/)";
...
export async function fetchAndCleanNewsBreakFeed(
  ...
  const channelTitle =
    sourceName ?? page.title ?? `NewsBreak · ${slug.replace(/-/g, " ")}`;
  ...
  const items: CleanedItem[] = page.feed
    ...
    .map((item) => {
      ...
      return {
        ...
        link: options.includeItemLinks
          ? `https://www.newsbreak.com/n/${item.docid}`
          : undefined,
        guid: item.docid,
